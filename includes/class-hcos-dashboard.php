...
final class HCOS_Dashboard {
	...
	public static function sidebar( $active = 'dashboard' ) {
		...
		$links = array(
			...
			array( 'bookings', 'Записи', admin_url( 'edit.php?post_type=lessons&page=hcos-bookings' ) ),
			...
			array( 'reports', 'Отчёты', admin_url( 'admin.php?page=hcos-reports' ) ),
	...
	private static function attention( $tasks ) {
		...
		</div><div class="hcos-quick-actions"><h3>Быстрые действия</h3><a class="is-primary" href="<?php echo esc_url( admin_url( 'post-new.php?post_type=clients' ) ); ?>">＋ Новый клиент</a><?php if ( current_user_can( 'hcos_view_finances' ) ) : ?><a href="<?php echo esc_url( admin_url( 'post-new.php?post_type=payments' ) ); ?>">Принять оплату</a><?php endif; ?><?php if ( current_user_can( 'edit_hcos_memberships' ) ) : ?><a href="<?php echo esc_url( admin_url( 'post-new.php?post_type=memberships' ) ); ?>">Создать абонемент</a><?php endif; ?></div></aside>
		<?php
	}
